export const Validators = {
    email :   (email)=>{
        ...
        return regex.test(email).valid;
    },
    password : (password) => password.trim() && (password.length>=5 && password.length<=20),
    age:        (age) => Number(age) && age>0 && age<100,
    phone:     (phone) => Number(phone) && String(phone).length === 9
}